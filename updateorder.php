...
if (!$con){
    ...
    $query="SELECT `id`,`userid`, `productid`, `shippingaddress`, `totalprice`, `title`, `username`, `quantity`, `orderstatus`, `paymode`, `datePurchased` FROM swapdb.order"; //SQL statement to read the information
    ...
    if($nrows>0){
        ...
        While($row=$result->fetch_assoc()){ //fetch assoc allows you to read in a record and allows you to traverse your results row by row
            ...
            echo "</td>";
            echo "<td>";
            echo "<a href='updatedeliverystatus.php?id=$id'> Update status </a>";
            echo "</td>";
            echo "</td>";
            echo "<td>";
            echo "<a href='deleteorder.php?id=$id'> Delete order </a>";
            echo "</td>";
            echo "</tr>";
        }
        echo "</table>";
